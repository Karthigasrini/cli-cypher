...
## The protocol of CYPHER is defined by Google Protobuf and contains a range of layers, from account, block to transfer.
...
```
     ...
     `asset`: other assets expect CYP in this account – e.g. _{<“WishToken”,66666>,<”Dogie”,233>}_.
...
```    
     ...
     `amount`: the amount of CYP – e.g. _12534_.  
...
```
     ...
     `cyp_num`: the number of CYPHER – e.g._232241_.  
     ...
     `description`: the description of this contract – e.g.”_cypherdada_”.  
...
+	Inventory is mainly used to inform peer nodes the list of items.  
    ...
    `type`: what type this `Inventory` is. – e.g. _0_ stands for `CYP`.  
    `ids`: ID of things in this `Inventory`.

    Two `Inventory` types: `CYP` and `BLOCK`.  
    `CYP`: transaction.  
    `BLOCK`: block.
    ...
    `type`: type of items – e.g. _1_ stands for `CYP`.  
    ...
    `Items` have four types: `ERR`, `CYP`, `BLOCK` and `BLOCKHEADER`.  
    `ERR`: error.  
    `CYP`: transaction.  
    `BLOCK`: block.  
